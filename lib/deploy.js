...
const { loadJSON, saveJSON, bytesToKB } = require('./utility');
...
function installModules(functionsDir) {
  ...
  // const { code, stderr } = sh.exec(`cd '${functionsDir}' && npm i`);
  // if (code !== 0) {
...
async function getDependencySize(name, version) {
  const data = await getSizes(name + '@' + version, { cwd: true });
  console.log('size', name, data.size);
  return bytesToKB(data.size);
  // const dependency = await libnpm.packument(name);
  // for (const currentVersion in dependency.versions) {
  //   if (version.endsWith(currentVersion)) {
  //     const currentPackage = dependency.versions[currentVersion];
  //     return bytesToKB(currentPackage.dist.unpackedSize);
  //   }
  // }
  // throw new Error(`Cannot retrieve data about the size of the package ${name}@${version}`);
}


async function getDependenciesInfo(packageJSON) {
  const dependencies = {
  ...
  const promises = Object.keys(dependencies).map(async (name) => {
...
async function prunePackages(rootDir, packageJSON, dependenciesInfo, totalSize) {
  ...
  const prunedDependenciesInfo = Object.keys(prunedDependencies).map((name) => {
    return {
      name,
      version: prunedDependencies[name],
      size: dependenciesInfo[name].size,
    }
  });

  const prunedSize = prunedDependenciesInfo.reduce((totalSize, dependency) => {
    if (!isNaN(size)) {
      return totalSize + dependency.size;
    } else {
      return totalSizel
    }
  }, 0);
  ...
  console.debug(`Dependencies size: ${totalSize}KB -> ${prunedSize}KB (${totalSize - prunedSize}KB saved)`);
  return {
    prunedSize,
    dependencies: prunedDependenciesInfo,
    prunedPackage: {
      ...packageJSON,
      dependencies: {
        ...prunedDependencies,
        // TODO: Test v8 compile cache speedup
        // Uncomment to enable v8 compile cache speedup
...
function removeUnusedFiles(functionsDir) {
  ...
  console.debug('Unreachable files removed:', removed, '/', stdout);
}
...
async function deploy(rootDir, functions, debug) {
  ...
  const triggers = (await getTriggers(functionsDir, projectId)).filter((trigger) => {
  ...
  const { totalSize, dependenciesInfo } = await getDependenciesInfo(packageJSON);

  console.log('totasizel', totalSize);
  console.log('info', dependenciesInfo);
  ...
  try {
    for (const subproject of subprojects) {
      try {
        ...
        const { prunedPackage, dependencies, prunedSize } = await prunePackages(subproject.movedFunctionsDir, packageJSON, dependenciesInfo, totalSize);
        sh.rm(path.resolve(subproject.movedFunctionsDir, 'package-lock.json'));
        ...
        // TODO: Test v8 compile cache speedup
        // Uncomment to enable v8 compile cache speedup
        // const libRootPath = path.resolve(subproject.movedFunctionsDir, 'lib', 'index.js');
        // sh.exec(`echo "require('v8-compile-cache');\n$(cat '${libRootPath}')" > '${libRootPath}'`);
        ...
        const { timestamp } = deploySubproject(subproject.movedFunctionsDir, subproject.trigger.entryPoint, projectId, debug);
        ...
        const deploy = regions.map((region) => ({
          projectId,
