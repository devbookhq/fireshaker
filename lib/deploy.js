...
function bundleProject(functionsDir) {
  const libDir = path.resolve(functionsDir, 'lib');
  const distDir = path.resolve(functionsDir, 'dist');
  const libRoot = path.resolve(libDir, 'index.js');

  // const parcelExecutable = require.resolve('parcel');

  // const { code, stderr } = sh.exec(`npx ${parcelExecutable} build '${libRoot}' --no-source-maps --target node && find '${libDir}' -type f -exec rm -f {} + && mv -v '${distDir}/*' '${libDir}/'`);
  ...
  // const { code, stderr } = sh.exec(`find '${libDir}' -type f -exec rm -f {} + && mv -v '${distDir}/*' '${libDir}/'`);
  // if (code !== 0) {
  //   throw new Error(`Exit code ${code}: ${stderr}`);
  // }
}
  ...
  try {
    for (const subproject of subprojects) {
      try {
        ...
        sh.rm(path.resolve(subproject.movedFunctionsDir, 'lib'));
        ...
        // bundleProject(subproject.movedFunctionsDir);
