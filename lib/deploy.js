...
function removeUnusedFiles(functionsDir) {
  ...
  const output = depcruise([path.resolve(functionsDir, 'src', 'index.ts')], {
...
function deploySubproject(functionsDir, entryPoint, projectId) {
  ...
  console.error(stderr);
  return {
    deployTimestamp: new Date().valueOf(),
  };
}

function getFunctionUrl(trigger, projectId) {
  if (trigger.httpsTrigger === {}) {
    if (!trigger.regions) {
      return [`https://us-central1-${projectId}.cloudfunctions.net/${trigger.name}`];
    }
    return trigger.regions.map(region => `https://${region}-${projectId}.cloudfunctions.net/${trigger.name}`);
  }
}
...
async function deploy(rootDir, functions) {
  ...
  const promises = subprojects.map(async (subproject) => {
    ...
    const { deployTimestamp } = deploySubproject(subproject.movedFunctionsDir, subproject.trigger.entryPoint, projectId);
