...
function removeUnusedFiles(functionsDir) {
  const { totalFiles } = sh.exec(`find '${functionsDir}/lib//.' ! -name . -print | grep -c //`, { silent: true })
  const indexPath = path.resolve(functionsDir, 'lib', 'index.js');
  const output = depcruise([path.resolve(functionsDir, 'lib')], {
...
function isolateEntryPoint(functionsDir, entryPoint) {
  const rootPath = path.resolve(functionsDir, 'lib', 'index.js');
  const rawSource = fs.readFileSync(rootPath, 'utf8');
  const parsed = esprima.parseModule(rawSource);
  parsed.body.forEach((stat) => {
    stat
  });
  // parsed.body.push(esprima.parse('end()'));
  const newCode = escodegen.generate(parsed);
  fs.writeFileSync(rootPath, newCode);
}
