...
function deploySubproject(functionsDir, entryPoint, projectId, debug) {
  const { stdout, stderr, code } = sh.exec(`cd '${functionsDir}' && firebase deploy ${debug ? '--debug' : ''} --project ${projectId} --only functions:${entryPoint}`);
  console.log('Firebase exit code');
  if (code !== 0) {
    throw new Error(`Exit code ${code}: ${stderr}`);
  }
  console.log('Firebase stdout', stdout);
  return {
