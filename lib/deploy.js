...
async function deploy(rootDir, functions) {
  ...
  if (functions && functions.length !== triggers.length) {
    ...
    throw new Error(`Cannot find all functions that should have been deployed\n missing functions: ${missingFunctions.joint(' ')}`);
  }

  if (!functions && triggers.length === 0) {
    throw new Error('No Cloud Functions found in the project');
  }
